...
※ この記事は、誤字脱字、表現の冗長さ・曖昧さのチェックのためにClaude Codeを利用している。
...
- コンテキストが膨張するとOOMによるクラッシュ（MLX）や推論崩壊（llama.cpp）が起きる。これは設計の問題であり、パラメーター調整で解決できるものではない。
...
ただし、この設計を考えるにあたって、1つの前提がある。過去にAIにノート操作を任せた結果、Zettelkastenがノイズだらけになるという失敗を経験している（『[Zettelkastenで管理する情報は何で、どう管理すべきなのか](../../../2026/01/25/)』、『[Zettelkastenを機能させ、また形骸化させないために、何でもかんでも入れるべきではない。](../../../2026/01/31/)』）。その経験から、「AIはZettelkastenを読むことはできるが書いてはいけない」という原則をすでに確立している。今回の設計はその原則の上に成り立っている。
...
どちらが良い悪いではなく、設計思想のトレードオフだ。しかし、いずれにしてもコンテキストを膨張させないことが根本的な解決策であり、制約付きグラフ探索はその手段の1つになる。

## 課題

制約付きグラフ探索は1つの解だが、リンクだけを整備すれば十分というわけではない。ノート内のどの部分がAIにとって「文脈」として有効なのか、何をZettelkastenにおけるメタデータに含めるべきか、ノート自体にどのようなメタデータを持たせるかといった設計の問いは、リンクに対して型付きエッジを付与した後にも残る。**型付きエッジの付与は「設計行為」であり、どこを付与し、どこをしないかという全体像を先に決めなければ、部分最適に終わる** という考えである。

また、既存の大量のノートに型付きエッジを後から付与するコストも現実的な障壁だ。セマンティックウェブが普及しなかった理由の1つが「既存データへのメタデータ付与コスト」だったこと、GraphRAGが自動で検出を試みていることの2つを考えると、新規ノートから型付きエッジを付け始めて様子を見るスモールスタートが現実的だろうと考えている。
...
- コンテキスト膨張はOOMクラッシュや推論崩壊という実害を生む。これはパラメーター調整ではなく、設計で解決すべき問題である。
