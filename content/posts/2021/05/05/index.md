...
1. アップデート完了。不具合として認識しているのは、(1)Virt Managerを使った場合のUSB Redirectionの動作不良（[詳細](https://fedoraproject.org/wiki/Common_F34_bugs#USB_devices_forwarded_from_the_host_to_a_guest_aren.27t_recognized_in_a_libvirt_virtual_machine)）、(2) Pipewireがレッツノート上で機能しない（音が出ない）、の2点。
